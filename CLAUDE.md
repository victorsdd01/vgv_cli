...
- Tests en `test/` (8 archivos, 71 tests: project config, version checker, vgv config/presets, feature generator, model generator, api generator, flutter toolchain/FVM, add runner)
...
### 15. Frames: bundle amplio + biblioteca local (✅ HECHO, 2026-08-22)
Decisión del usuario tras ver los números: el set completo de Apple Product Bezels que descargó son **178 frames / ~130 MB** → **imposible bundlear todo** (base64 full-calidad ~104 MB > tope 100 MB/versión de pub.dev; reescalado fuerte = frames blandos a tamaño store). Solución elegida: **bundle amplio curado + biblioteca local**.
- **Bundle curado (~38)**: `tool/frames_builtin/` ahora tiene ~38 PNGs curados (Portrait primero, 1-2 colores por modelo popular, iPhone/iPad/Watch/MacBook/iMac/Studio Display/Apple TV), reescalados a 1200 → `builtin_frames.dart` (~6.2 MB base64, viaja a todos). Reemplazó el set inicial de 10.
- **Biblioteca local `~/.vgv/frames`**: comando nuevo **`vgv screenshots frames <dir>`** (`lib/core/utils/frames_extractor.dart`, ruteado en `screenshot_runner.dart`). Monta los `.dmg` (macOS, `printf 'Y\n' | hdiutil attach … -mountpoint`, auto-acepta SLA), extrae los PNGs bajo `PNG/`, reescala ≤2000 (paquete `image`, alpha preservado) y escribe a `~/.vgv/frames` (default = `defaultFramesDir()` en `screenshot_web.dart`). También acepta carpetas de `.png` sueltos (cross-platform). El server **auto-carga** `~/.vgv/frames` en `/api/frames` (mergea `--frames` + biblioteca + built-in, dedup, ordenado, con `credit`) y sirve `/frames/<name>` desde cualquiera de los dos dirs. Full calidad sin inflar el paquete publicado.
- **Editor**: cada thumb muestra su **nombre** (`.cap`), y las **chips de device filtran** los frames (`frameCategory(name)` + `filterFrames()`): iPhone→solo iPhones, iPad→iPads, etc.; Mac/iMac/Studio Display/Apple TV caen bajo la chip **MacBook**; hint de vacío (`frame_empty`, i18n EN/ES) cuando un device no tiene frames. Re-embebido (`generate_editor_html.dart`).
- Verificado end-to-end en browser: 178 en `/api/frames`, filtro iPad → 40 thumbs (solo ipad) con labels, canvas store-exacto 2064×2752 al elegir un iPad, frame real compone. Comando `frames` probado con `.dmg` reales (monta, SLA, extrae, nombres correctos). `analyze` limpio, 49 tests. `.pubignore` (tool/, test/) mantiene el paquete liviano.

### 16. Biblioteca de frames en la nube (`--cloud`) (✅ HECHO, 2026-08-22)
Para que **todos** tengan los 178 sin bundlear ~130 MB: repo público aparte **`victorsdd01/vgv_cli_frames`** (creado con `gh`) con `frames/` (178 PNG ≤2000px) + `manifest.json` (`{version,base,credit,count,frames:[{name,file,category}]}`) + `NOTICE`/`README` de atribución, tag **`v1`**, servido por **jsDelivr** (`https://cdn.jsdelivr.net/gh/victorsdd01/vgv_cli_frames@v1/...`). jsDelivr **solo sirve repos públicos** (privado no sirve; token en el CLI publicado = mala práctica). El upload lo hizo **el usuario** (el classifier me bloquea publicar assets de Apple; le pasé los comandos: `gh auth setup-git` para el credential helper, commit, push, tag).
...
- **Nombre de la app (pedido del usuario)**: antes se derivaba de la carpeta (`_humanizeName`) sin preguntar. Ahora `ProjectConfig` tiene `appName` opcional + getter `effectiveAppName` (null → humaniza `projectName`, o sea el flujo de creación no cambia), y el datasource usa ese getter en Android e iOS. `add flavors` **detecta** el nombre real (`android:label`, siguiendo `@string/...` a `strings.xml`; fallback al `CFBundleDisplayName`/`CFBundleName` del Info.plist ignorando `$(...)`), lo muestra y **pregunta si conservarlo** (solo agrega el sufijo por flavor) **o cambiarlo**. Flag `--app-name` para saltear el prompt.
...
- ⚠️ **Probar prompts es difícil**: bajo un pty casero (y bajo `dart run`) `stdout.hasTerminal` da false, así que mason_logger tira `StateError` y el código degrada a no-interactivo — el pty **no** sirve para verificar las ramas interactivas. Se testean con un **Logger stub** (`_ScriptedLogger` en `test/add_runner_test.dart`) que sobreescribe `confirm`/`prompt`. Ese test encontró un bug real: `_writeEntryPoints` explotaba si no existía `lib/` (ahora lo crea).
